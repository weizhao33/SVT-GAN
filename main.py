...
def test_loop(net, data_loader, num_iter):
    ...
    with torch.no_grad():
        ...
        for rain, norain, name, h, w in test_bar:
            ...
            out = torch.clamp(model(rain), 0, 1)

            # computer the metrics with Y channel and double precision
            y, gt = rgb_to_y(out.double()*255.0), rgb_to_y(norain.double()*255.0)
            # y, gt = out.double(), norain.double()
            current_psnr, current_ssim = psnr(y, gt), ssim(y, gt)
            total_psnr += current_psnr.item()
            total_ssim += current_ssim.item()
            count += 1
            save_path = '{}/{}/{}'.format(args.save_path, args.data_name, name[0])
            read_path = '{}/{}/{}'.format('./data/rain100H/test', 'rain', name[0])
            if not os.path.exists(os.path.dirname(save_path)):
                os.makedirs(os.path.dirname(save_path))
            # print(read_path)
            ds = pydicom.dcmread(read_path)
            dt = out.double().cpu().numpy()
            dt = dt *3000.0
            # print(h)

            ds.PixelData = dt[0:1,:,:].astype(np.uint16).tobytes()
            ds.save_as(save_path, write_like_original=True)
