...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='./data/rain100H/test/rain/')
    parser.add_argument('--data_name', type=str, default='rain100H', choices=['rain100L'])
    ...
    parser.add_argument('--num_iter', type=int, default=20000, help='iterations of training')
    ...
    # parser.add_argument('--patch_size', nargs='+', type=int, default=[120, 160, 190, 250, 320, 380],
                        # help='patch size of each image for progressive learning')


    parser.add_argument('--channels', nargs='+', type=int, default=[48, 96, 192, 384],
                        help='number of channels for each level')
    parser.add_argument('--batch_size', nargs='+', type=int, default=[32, 10, 8, 4, 2, 2],
                        help='batch size of loading images for progressive learning')
    parser.add_argument('--patch_size', nargs='+', type=int, default=[64, 160, 190, 250, 320, 380],
                        help='patch size of each image for progressive learning')
    ...
    parser.add_argument('--model_file', type=str, default=None, help='./result/rain100H.pth')
...
def is_image_file(filename):
     return any(filename.endswith(extension) for extension in ['dcm','jpeg', 'JPEG', 'jpg', 'png', 'JPG', 'PNG', 'gif'])
...
class RainDataset(Dataset):
    def __init__(self, data_path, data_name, data_type, patch_size=None, length=None):
        ...
        # self.rain_images = sorted(glob.glob('{}/{}/{}/rain/*.dcm'.format(data_path data_name, data_type)))
        # self.rain_images = sorted(glob.glob('data/rain100H/test/rain/*.dcm'),key=lambda x:eval(x.split("/")[-1].split('.')[0]))

        input_image = glob.glob('data/rain100H/test/rain/*.dcm')  # 返回名称
        self.rain_images = sorted(input_image, key=lambda x: eval(os.path.basename(x).split(".")[0]))
        # print(self.rain_images )

        # self.norain_images = sorted(glob.glob('{}/{}/{}/norain/*.dcm'.format(data_path, data_name, data_type)))
        self.norain_images  = sorted(glob.glob('data/rain100H/test/norain/*.dcm'),key=lambda x: eval(os.path.basename(x).split(".")[0]))
        # print(self.norain_images)
        ...
        #
        # self.rain_images = [os.path.join(rgb_dir, 'input', x) for x in inp_files if is_image_file(x)]
        # self.norain_images = [os.path.join(rgb_dir, 'target', x) for x in tar_files if is_image_file(x)]
    ...
    #     self.num = len(self.norain_images)  # get the size of target
    #
    ...
    def __getitem__(self, idx):
        ...
        inp_path = self.rain_images[idx % self.num-2]
        tar_path = self.norain_images[idx % self.num-2]

        # JPG = T.to_tensor(Image.open('D:\DRSformer-main\Datasets\\test\Rain200H\input\\1.PNG'))
        # print(JPG.min())

        kk = np.zeros([512, 512, 3], np.float32)

        ds = pydicom.dcmread(self.rain_images[idx % self.num-2])
        kk[:,:,0]= ds.pixel_array.astype(np.float32)/3000.0

        ds = pydicom.dcmread(self.rain_images[idx % self.num-1])
        kk[:,:,1]= ds.pixel_array.astype(np.float32)/3000.0

        ds = pydicom.dcmread(self.rain_images[idx % self.num])
        kk[:,:,2]= ds.pixel_array.astype(np.float32)/3000.0
        # print(kk.max())
        rain = T.to_tensor(kk)
        # print(rain.min())
        # print(self.rain_images[idx % self.num-2])
        # print(self.rain_images[idx % self.num-1])
        # print(self.rain_images[idx % self.num])


        ds = pydicom.dcmread(self.norain_images[idx % self.num - 2])
        kk[:, :, 0] = ds.pixel_array.astype(np.float32)/ 3000.0

        ds = pydicom.dcmread(self.norain_images[idx % self.num - 1])
        kk[:, :, 1] = ds.pixel_array.astype(np.float32) / 3000.0

        ds = pydicom.dcmread(self.norain_images[idx % self.num])
        kk[:, :, 2] = ds.pixel_array.astype(np.float32) / 3000.0
        norain = T.to_tensor(kk)


        # print(norain.shape)   # 原来维度是多少？
        # rain = rain/3500*255
        # norain = norain/3500*255
        h, w = rain.shape[1:]
...
def ssim(x, y, kernel_size=11, kernel_sigma=1.5, data_range=255.0, k1=0.01, k2=0.03):
    ...
    f = max(1, round(min(x.size()[-2:]) / 512))
    if f > 1:
